...
// Query to fetch ring products
const PRODUCTS_QUERY = defineQuery(`*[_type == "product" && type == "ring" || category == "ring"]{
  name, 
  price, 
  description,
...
export default async function Rings(){
  ...
  try {
    ...
    console.log("Fetched ring products:", products)
    
    // Check if products exist and have items
    if (!products || products.length === 0) {
      return (
        <div className="container mx-auto py-8">
          <h1 className="text-3xl font-philosopher text-white mb-8 text-center">Rings</h1>
          <p className="text-center text-white">No rings found. Check back soon!</p>
        </div>
      )
    }
    ...
    return (
      <div className="container mx-auto py-8">
        <h1 className="text-3xl font-philosopher text-white mb-8 text-center">Rings</h1>
        <p className="text-center text-white">Sorry, we couldn't load the rings. Please try again later.</p>
      </div>
    )
  }
}